...
const serviceForm = () => {
    ...
    const physicians = dropdownData?.physicians?.map(
        (physician: { physician_name: string }) => 
            physician.physician_name) || [];

    const exams = dropdownData?.service?.map(
    ...
    async function handleSubmit(event: any) {
            ...
            try {
                ...
                if (res.data.success) {
                    setModalMessage("Case created successfully!");
                } else {
                    setModalMessage("Failed to create case.");
                }
            } catch (err) {
                console.error(err);
    ...
    function closeModal() {
            if (modalMessage === "Case created successfully!") {
                navigate(`/patients/${id}`);
            }
        ...
        <form className="add-service-content" onSubmit={handleSubmit}>
            ...
            <div className="form-grid">
                ...
                <div className="form-section">
                    ...
                    <select 
                        name="patientSource" 
                        value={formValues.patientSource} 
                        onChange={handleInput}
                        required>
                        <option value="">-- Select an Option --</option>
